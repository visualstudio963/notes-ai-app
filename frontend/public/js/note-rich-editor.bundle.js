...
3. "-" cannot repeat`);
  ...
  var index_default7 = Placeholder;
  ...
  function getHTMLFromFragment2(doc3, schema, options) {
    ...
    if (options == null ? void 0 : options.document) {
      const wrap22 = options.document.createElement("div");
      DOMSerializer.fromSchema(schema).serializeFragment(doc3.content, { document: options.document }, wrap22);
      return wrap22.innerHTML;
    }
    const wrap2 = window.document.createElement("div");
  ...
  function encodeDocForStorage(doc3) {
  ...
  function getToolbarButtons(editorInstance) {
    ...
    wrap2.appendChild(btn("Heading 1", svg.h1, () => chain().toggleHeading({ level: 1 }).run(), () => is("heading", { level: 1 })));
    wrap2.appendChild(btn("Heading 2", svg.h2, () => chain().toggleHeading({ level: 2 }).run(), () => is("heading", { level: 2 })));
    wrap2.appendChild(btn("Heading 3", svg.h3, () => chain().toggleHeading({ level: 3 }).run(), () => is("heading", { level: 3 })));
    wrap2.appendChild(btn("Bullet list", svg.bullet, () => chain().toggleBulletList().run(), () => is("bulletList")));
    wrap2.appendChild(btn("Numbered list", svg.number, () => chain().toggleOrderedList().run(), () => is("orderedList")));
    wrap2.appendChild(
      btn("Task list", svg.check, () => chain().toggleTaskList().run(), () => is("taskList"))
    );
    wrap2.appendChild(btn("Quote", svg.quote, () => chain().toggleBlockquote().run(), () => is("blockquote")));
    wrap2.appendChild(btn("Code block", svg.code, () => chain().toggleCodeBlock().run(), () => is("codeBlock")));
    wrap2.appendChild(
      btn("Link", svg.link, () => {
  ...
  function initNoteRichEditorBridge() {
    document.getElementById("noteRichEditorBack")?.addEventListener("click", () => {
      void runPersist().finally(() => close2());
    });
